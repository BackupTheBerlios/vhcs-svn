...
function emptytrash() {	location = 'folders.php?empty=".$sess["sysmap"]["trash"]."&folder=".urlencode($folder)."&goback=true&sid=$sid&tid=$tid&lid=$lid';}
...
for($n=0;$n<count($boxes);$n++) {
	...
	if(in_array($entry, $sess["sysfolders"])) {
		...
		$system[$scounter]["chlink"] 		= "process.php?folder=".$entry."&sid=$sid&tid=$tid&lid=$lid";
		$system[$scounter]["emptylink"]		= "folders.php?empty=".$entry."&folder=".$entry."&sid=$sid&tid=$tid&lid=$lid";
