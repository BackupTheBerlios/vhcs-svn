...
/* for status */
define('STATUS_OK','ok');
define('STATUS_TOADD','toadd');
define('STATUS_TODELETE','delete');
define('STATUS_TOCHANGE','change');
define('STATUS_TORESTORE','restore');
...
function translate_dmn_status ( $status ) {
    ...
    } else if ( $status == STATUS_TOADD) {

        return tr('wait to be added');

    } else if ( $status == STATUS_TOCHANGE) {

        return tr('wait to be modified');

    } else if ( $status == STATUS_TODELETE) {
...
SQL_QUERY;
    ...
    $subject = "Auto message allert for domain order {DOMAIN} !";
